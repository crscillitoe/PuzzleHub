...
export class HashiStandardComponent implements OnInit {
  ...
  private static numNodes: number;

  public static ngOnInitOverwrite(that) {
    that.loader.startLoadingAnimation();
    const diff = Number(that.route.snapshot.paramMap.get('diff'));
    that.diff = diff;
    that.difficulty = diff;

    HashiStandardComponent.setupBoard(that);
    ...
    if (that.userService.isLoggedIn()) {
      ...
      that.timer.startTimer(GameID.HASHI, that.diff)
        .subscribe( (data) => {
          that.seed = data['seed'];
          that.board = new Board(that.width, that.height, HashiStandardComponent.numNodes, that.extreme, that.seed, null, null, null, null, null, that.gauntlet, null);
          that.generateFairBoard(HashiStandardComponent.numNodes);
          this.play(that);
        });
    } else {
      that.seed = 0;
  ...
  public static setupBoard(that) {
    switch (that.difficulty) {
      case 1:
      default: {
        that.width = 10;
        that.height = 10;
        HashiStandardComponent.numNodes = 0;
        that.extreme = false;
        break;
      }
      case 2: {
        that.width = 15;
        that.height = 15;
        HashiStandardComponent.numNodes = 15000;
        that.extreme = false;
        break;
      }
      case 3: {
        that.width = 25;
        that.height = 25;
        HashiStandardComponent.numNodes = 500000;
        that.extreme = false;
        break;
      }
      case 4: {
    ...
    if (HashiStandardComponent.numNodes === 0) {
      HashiStandardComponent.numNodes = Math.floor(Math.sqrt(that.width * that.height)) * 2;
    }
  }
  ...
  public static newBoard(that) {
    ...
    HashiStandardComponent.setupBoard(that);
    that.numNodes = HashiStandardComponent.numNodes;
    console.log(HashiStandardComponent.numNodes);
      ...
      that.timer.startTimer(GameID.HASHI, that.diff)
        .subscribe( (data) => {
          ...
          that.board = new Board(
            that.width,
            that.height,
            HashiStandardComponent.numNodes,
            that.extreme,
            that.seed,
            null,
            null,
  ...
  public static play(that) {
    that.playing = true;
    const numNodes = Number(that.route.snapshot.paramMap.get('numNodes'));
  ...
  public static numBridgeRight(that, num) {
    ...
    for (counter = that.coloredNode.getX() + 1; counter < that.width + 1; counter++) {
      if (that.isCircleHere(counter, that.coloredNode.getY())) {
        let toBridgeTo = that.getCircleHere(counter, that.coloredNode.getY());
        ...
        if (!bridgeExists) {
          that.addConstructedBridges(num);
          let bridge = new Bridge(that.coloredNode, toBridgeTo, num);
          that.coloredNode.addBridge(bridge);
          toBridgeTo.addBridge(bridge);
        }
  ...
  public static drawGrid(that) {
    ...
    if (that.drawGridBool) {
      for (x = 1 ; x <= that.width ; x++) {
        circleX = that.xAdd + (x * (that.factor));
        circleY = that.yAdd + (1 * (that.factor));
        circleY2 = that.yAdd + (that.height * (that.factor));
      ...
      for (y = 1 ; y <= that.height ; y++) {
        circleX = that.xAdd + (1 * (that.factor));
        circleY = that.yAdd + (y * (that.factor));
        circleX2 = that.xAdd + (that.width * (that.factor));
